...
const EntireAssets = () => {
  ...
  const fetchAssetData = async () => {
    try {
      ...
      const res = await fetch(
        `http://192.168.1.3:1443/api/CategoryAndAsset/GetCategoriesByIDs?OutletID=${filteredoutletvalue}&CategoryID=${filteredcategoryvalue}`
      )
      if (!res.ok) {
  ...
  const fetchOutlets = async () => {
    try {
      ...
      setLoading(true)
      const res = await fetch(
        `http://192.168.1.3:1443/api/AssetOutlet/GetAllOutlets`
      )
      if (!res.ok) {
